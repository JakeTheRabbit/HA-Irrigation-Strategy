...
async def test_a_blank_install_gets_the_entity_ids_the_controller_looks_for(hass):
    ...
    ours = [e.entity_id for e in er.async_entries_for_config_entry(registry, entry.entry_id)]
    assert len(ours) > 100
    strays = [eid for eid in ours if not eid.split(".", 1)[1].startswith("crop_steering_")]
    assert strays == [], f"named from their labels, invisible to the controller: {strays[:8]}"
...
async def test_the_controller_adopts_a_freshly_installed_tent_and_waters_it(hass, controller_for):
    ...
    assert room.hw == {"pump": None, "mainline": None, "valves": {1: TENT_SWITCH},
                       "plumbing": "valves_only"}
    assert room.setup_revision == 1 and room._setup_pending is None  # adopted: everything read OFF
    ...
    for switch in (KILL, "switch.crop_steering_system_enabled",
...
async def test_the_controller_reads_the_tents_microsiemens_probe_as_millisiemens(hass, controller_for):
    await _install_tent(hass)
    c, _fake, _clock = controller_for({"enable_flag": KILL})
    fused = c._fused_id("", "ec", 1) if hasattr(c, "_fused_id") else "sensor.crop_steering_ec_zone_1"
    assert fused == "sensor.crop_steering_ec_zone_1"
    assert c._read_sensor(fused, lo=0, hi=20) == pytest.approx(2.3)


async def test_a_room_whose_layout_needs_a_pump_is_held_until_the_pump_is_mapped(hass, controller_for):
    ...
    hass.states.async_set("sensor.crop_steering_engine_config", "default",
    ...
    assert block and ("no hardware mapped" in block or "Invalid setup" in block or "Setup" in block)
