...
async def test_a_catch_test_sets_the_dripper_flow_the_engine_reads(hass):
    ...
    assert float(hass.states.get(FLOW).state) == pytest.approx(
        3.785, abs=1e-3
    )  # the packet: 1 GPH
    ...
    assert (
        done["type"] is FlowResultType.ABORT and done["reason"] == "calibration_applied"
    )
    assert (
        "0.79 gal/hr" in done["description_placeholders"]["flow"]
    )  # reported in the grower's unit
...
async def test_an_implausible_catch_test_is_refused_and_nothing_is_written(hass):
    ...
    assert flow["type"] is FlowResultType.FORM and flow["errors"] == {
...
async def test_editing_a_parameter_changes_what_the_engine_reads(hass):
    ...
    assert (
        shown["substrate_volume"] == 11.4
    )  # the LIVE value, not a stale copy from setup
    done = await hass.config_entries.options.async_configure(
        flow["flow_id"],
        {
            "substrate_volume": 11.4,
            "dripper_flow_rate": 3.785,
            "p1_target_vwc": 61.0,
...
async def test_editing_while_the_switch_is_on_is_an_inline_error_that_keeps_the_edit(
    hass,
):
    entry = await _install_tent(hass)
    ...
    assert {str(m): m.default() for m in flow["data_schema"].schema}[
        "p1_target_vwc"
    ] == 61.0
    assert (
        float(hass.states.get("number.crop_steering_p1_target_vwc").state) == 65.0
    )  # not applied
...
async def test_configure_reports_a_wrong_layout_beside_the_field_instead_of_aborting(
    hass,
):
