...
async def _upgrade(hass, name, *, registry_ids=None):
    ...
    entry = MockConfigEntry(
        domain=DOMAIN, entry_id=seed["entry_id"], unique_id=seed["unique_id"],
        title=seed["title"], data=seed["data"], options=seed["options"],
    )
    ...
    for entity_id, unique_key in (registry_ids or {}).items():
        ...
        registry.async_get_or_create(
            platform, DOMAIN, f"{DOMAIN}_{seed['entry_id']}_{unique_key}",
...
def _known_numbers(seed):
    return {eid: eid.split("crop_steering_", 1)[1] for eid in seed["operator_tuned_numbers"]}
...
async def test_an_entity_the_operator_renamed_keeps_its_name_and_is_not_duplicated(hass):
    ...
    entry, seed = await _upgrade(
        hass, "entry_2_17_wizard.json",
        registry_ids={"sensor.flower_row_1_moisture": "vwc_zone_1",
...
async def test_none_of_the_new_features_switch_themselves_on(hass):
    ...
    assert (descriptor["pump"], descriptor["mainline"]) == ("switch.main_pump", "switch.main_line")
    ...
    assert hass.states.get("sensor.crop_steering_zone_1_suggested_field_capacity").state == "unknown"
...
async def test_the_controller_carries_straight_on_after_the_upgrade_without_a_disarm_cycle(
    ...
    saved_by_0_14 = json.dumps({
        "active": True, "zones": [1, 2], "pump": "switch.main_pump", "mainline": "switch.main_line",
        "valves": {"1": "switch.row_1_valve", "2": "switch.row_2_valve"}, "enable_flag": KILL,
        "feed_ec_sensor": "sensor.feed_ec", "feed_ph_sensor": "sensor.feed_ph",
    }, sort_keys=True)
    ...
    assert room.state[1]["shots"] == 7 and room.state[1]["daily_vol"] == 9.5  # counters kept
    ...
    for switch in ("switch.crop_steering_system_enabled",
                   "switch.crop_steering_auto_irrigation_enabled", "switch.crop_steering_zone_1_enabled"):
        fake.set_state(switch, "on")
    fake.calls.clear()
    c._execute_shot(room, 1, 10, 5, flow_lps=0.05)
    assert switch_calls(fake) == [
        ("turn_on", "switch.main_pump"), ("turn_on", "switch.main_line"), ("turn_on", "switch.row_1_valve"),
        ("turn_off", "switch.row_1_valve"), ("turn_off", "switch.main_line"), ("turn_off", "switch.main_pump")]
...
async def test_configure_offers_an_old_room_the_layout_it_already_runs_as(hass):
    ...
    flow = await hass.config_entries.options.async_configure(flow["flow_id"], {"next_step_id": "edit_zones"})
    flow = await hass.config_entries.options.async_configure(flow["flow_id"], {"num_zones": 2})
    assert flow["step_id"] == "edit_zones_map"
    defaults = {str(marker): marker.default() for marker in flow["data_schema"].schema
...
async def test_an_env_era_install_with_only_front_and_back_probes_still_loads_and_fuses(hass):
    seed = fixture("entry_env_era.json")
    entry, _ = await _upgrade(hass, "entry_env_era.json", registry_ids=_known_numbers(seed))
    assert float(hass.states.get("sensor.crop_steering_vwc_zone_1").state) == 60.0  # (58 + 62) / 2
...
async def test_a_probe_with_no_unit_on_an_old_install_reads_exactly_as_it_always_did(hass):
    ...
    assert float(hass.states.get("sensor.crop_steering_ec_zone_1").state) == pytest.approx(3.2)


async def test_the_controller_still_requires_all_three_switches_of_an_old_room(hass, controller_for):
    ...
    c, _fake, _clock = controller_for({"enable_flag": "input_boolean.f2_control_enabled"})
    assert c.rooms[0].hw["pump"] == "switch.veg_pump" and "plumbing" not in c.rooms[0].hw
...
async def test_a_second_room_can_still_be_added_beside_an_upgraded_default_room(hass):
    ...
    flow = await hass.config_entries.flow.async_configure(flow["flow_id"], {"room_name": "Veg tent"})
    assert flow["step_id"] == "manual_zones"
    flow = await hass.config_entries.flow.async_configure(
        flow["flow_id"], {"num_zones": 1, "plumbing": "valves_only", "volume_unit": "L", "flow_unit": "L/hr"})
    flow = await hass.config_entries.flow.async_configure(
        flow["flow_id"], {"ec_unit": "auto", "zone_1_switch": "switch.veg_tent",
                          "zone_1_vwc": ["sensor.veg_vwc"], "zone_1_plant_count": 2})
