...
export type Tone = "normal" | "high" | "over" | "muted";
...
export function Meter({
  ...
  const range = from !== null && from !== undefined;
  const start = range ? share(from, max) : 0;
  return (
    <span className="meter" role="img" aria-label={label}>
      <span
        ...
        style={{ marginLeft: `${start}%`, width: `${Math.max(0, share(value, max) - start)}%` }}
      />
...
/** Green is on or good, red off or failed, amber needs attention, blue is water, violet a phase
 * change; neutral and unknown are grey. */
export type PillTone = "on" | "off" | "warn" | "water" | "phase" | "neutral" | "unknown";

/** A rounded state pill. The dot marks a state that can change on its own, like a valve. */
export function Pill({
  tone,
  dot = false,
  children,
  ...rest
}: ComponentProps<"span"> & { tone: PillTone; dot?: boolean }) {
  return (
