...
def test_old_partial_state_keeps_known_drops_unknown_seeds_new(tmp_path):
    ...
    p.write_text(json.dumps({"1": {"phase": "P1", "peak": 55.0, "legacy_removed_field": 7}}), encoding="utf-8")
    c = _make([1, 2], p)
    st = c._load_state()
...
def test_bad_timestamp_is_tolerated(tmp_path):
    p = tmp_path / "state.json"
    p.write_text(json.dumps({"1": {"phase": "P3", "last_shot": "not-a-date"}}), encoding="utf-8")
    c = _make([1], p)
    st = c._load_state()  # must not raise
