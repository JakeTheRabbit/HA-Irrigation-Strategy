...
def _tent_payload(**extra):
    return {
        ...
        "zones": [{"id": 1, "valve": TENT, "vwc_sensors": ["sensor.gt1_vwc"],
                   "ec_sensors": ["sensor.gt1_ec"], "plant_count": 4}],
        "hardware": {},
        **extra,
    }
...
def test_a_switch_that_is_on_is_reported_as_on():
    ...
    assert api.safety_blockers(hass, proposed=data) == [
        f"{TENT} must read OFF before changing setup"]  # the sentence callers already know


@pytest.mark.parametrize("state, says", [
    ("unavailable", "offline"), ("unknown", "has not reported"), ("standby", "not reporting on/off")])
...
def test_the_wizard_shows_the_on_switch_beside_its_picker_and_keeps_every_answer(flow_module):
    hass = _hass(_tent_states(switch="on"))
    flow = flow_module.ConfigFlow()
    flow.hass = hass
    flow._data.update(name="Grow tent 1", room_name="Grow tent 1", room_prefix="")
    asyncio.run(flow.async_step_manual_zones(
        {"num_zones": 1, "plumbing": "valves_only", "volume_unit": "gal", "flow_unit": "gal/hr"}))
    answers = {"ec_unit": "auto", "zone_1_name": "Tent", "zone_1_active": True, "zone_1_switch": TENT,
               "zone_1_vwc": ["sensor.gt1_vwc"], "zone_1_ec": ["sensor.gt1_ec"], "zone_1_plant_count": 4}
    ...
    assert flow._data["zones"]["1"]["vwc_sensors"] == ["sensor.gt1_vwc"]  # nothing typed is lost
...
def test_a_single_switch_tent_completes_the_wizard_in_its_own_units(flow_module):
    ...
    flow._data.update(name="Grow tent 1", room_name="Grow tent 1", room_prefix="")
    run = asyncio.run
    run(flow.async_step_manual_zones(
        {"num_zones": 1, "plumbing": "valves_only", "volume_unit": "gal", "flow_unit": "gal/hr"}))
    step = run(flow.async_step_zones(
        {"ec_unit": "auto", "zone_1_switch": TENT, "zone_1_vwc": ["sensor.gt1_vwc"],
         "zone_1_ec": ["sensor.gt1_ec"], "zone_1_plant_count": 4}))
    ...
    step = run(flow.async_step_substrate(
        {"substrate_preset": "pot_3gal", "substrate_volume": 1, "dripper_flow_rate": 1,
         "drippers_per_plant": 2, "field_capacity": 70, "max_ec": 9, "have_catch_test": False}))
    ...
    assert data["hardware"]["pump_switch"] == "" and data["hardware"]["main_line_switch"] == ""
    assert data["parameters"]["substrate_volume"] == 11.4  # the 3 gal preset, not the typed 1
    assert data["parameters"]["dripper_flow_rate"] == pytest.approx(3.785, abs=1e-3)  # 1 GPH
    assert data["parameters"]["lights_on_hour"] == 6
    descriptor = room.build_engine_config("", "default", 1, data["zones"], data["hardware"], data)
...
def test_a_catch_test_in_the_wizard_replaces_the_packet_rating(flow_module):
    ...
    flow._data.update(parameters={"dripper_flow_rate": 2.0}, flow_unit="L/hr")
    bad = asyncio.run(flow.async_step_catch_test(
        {"catch_seconds": 10, "catch_ml": 5000, "catch_drippers": 1}))
    assert bad["step_id"] == "catch_test" and bad["errors"] == {"base": "catch_test_out_of_range"}
    assert flow._data["parameters"]["dripper_flow_rate"] == 2.0  # a nonsense result is not saved
    done = asyncio.run(flow.async_step_catch_test(
        {"catch_seconds": 60, "catch_ml": 200, "catch_drippers": 4}))
    assert done["step_id"] == "extras" and "3 L/hr" in done["description_placeholders"]["measured"]
...
def test_a_size_outside_the_limits_is_explained_in_the_growers_own_unit(flow_module):
    flow = flow_module.ConfigFlow()
    flow.hass = _hass(_tent_states())
    flow._data.update(volume_unit="gal", flow_unit="gal/hr")
    step = asyncio.run(flow.async_step_substrate(
        {"substrate_preset": "custom", "substrate_volume": 80, "dripper_flow_rate": 1,
         "drippers_per_plant": 1, "field_capacity": 70, "max_ec": 9, "have_catch_test": False}))
...
def test_both_layers_agree_on_what_each_layout_needs():
    ...
    sys.modules.setdefault("requests", SimpleNamespace(Session=lambda: SimpleNamespace(headers={})))
    try:
        import controller
    finally:
...
def test_a_layout_is_a_promise_checked_both_ways():
    ...
    assert (needs.value.key, needs.value.path) == ("plumbing_needs_pump", ("hardware", "pump_switch"))
    with pytest.raises(api.SetupError) as unused:
        api.prepare_setup(_hass(states), _tent_payload(plumbing="valves_only",
...
def test_one_switch_cannot_be_both_the_zone_and_the_pump_and_the_error_says_what_to_do_instead():
    with pytest.raises(api.SetupError) as error:
        api.prepare_setup(_hass(_tent_states()), _tent_payload(hardware={"pump_switch": TENT}))
    assert error.value.key == "valve_is_shared" and error.value.path == ("hardware", "pump_switch")
    assert str(error.value) == "A valve cannot also be pump, mainline or waste"  # unchanged sentence
...
@pytest.mark.parametrize("unit, problem", [("ppm", "ec_unit_ppm_scale"), (None, "ec_unit_missing"),
                                           ("%", "ec_unit_unknown")])
def test_an_ec_probe_that_cannot_be_interpreted_is_flagged_on_its_own_field(unit, problem):
    with pytest.raises(api.SetupError) as error:
...
def test_choosing_the_scale_is_what_lets_a_ppm_probe_through():
    data = api.prepare_setup(_hass(_tent_states(ec_unit="ppm")), _tent_payload(ec_unit="ppm_700"))
    assert data["ec_unit"] == "ppm_700"


def test_the_feed_probes_factor_is_saved_and_published_for_the_controller():
    states = {**_tent_states(), "sensor.feed": ("2300", {"unit_of_measurement": "uS/cm"})}
    data = api.prepare_setup(_hass(states), _tent_payload(plumbing="valves_only",
                                                          hardware={"feed_ec_sensor": "sensor.feed"}))
    assert data["feed_ec_factor"] == pytest.approx(0.001)
    descriptor = room.build_engine_config("", "default", 1, data["zones"], data["hardware"], data)
    assert descriptor["feed_ec_factor"] == pytest.approx(0.001)
    cleared = api.prepare_setup(_hass(states), _tent_payload(hardware={"feed_ec_sensor": ""}), data)
    assert cleared["feed_ec_factor"] == 1.0  # unmapping the probe must not leave its factor behind
...
def _legacy_room():
    ...
    return {
        "room_name": "F2", "name": "F2", "room_prefix": "", "room_slug": "default", "active": True,
        "num_zones": 1, "setup_revision": 4,
        "zones": {"1": {"zone_number": 1, "name": "Row 1", "active": True, "zone_switch": "switch.v1",
                        "vwc_sensors": ["sensor.vwc"], "ec_sensors": ["sensor.ec"],
                        "vwc_front": "sensor.vwc", "vwc_back": "", "ec_front": "sensor.ec",
                        "ec_back": "", "plant_count": 36}},
...
def _legacy_hass():
    return _hass({"switch.v1": ("off", {}), "switch.p": ("off", {}), "switch.m": ("off", {}),
...
def test_re_saving_an_existing_room_adds_none_of_the_new_fields():
    old = _legacy_room()
    saved = api.prepare_setup(_legacy_hass(), api.configuration_payload(old), old, "one")
...
def test_an_existing_rooms_descriptor_is_exactly_what_it_published_before():
    ...
    old = _legacy_room()
    descriptor = room.build_engine_config("", "default", 1, old["zones"], old["hardware"], old)
...
def test_an_undeclared_layout_makes_no_demands_so_an_old_room_with_no_pump_still_saves():
    old = _legacy_room()
    old["hardware"] = {}
    assert api.prepare_setup(_legacy_hass(), api.configuration_payload(old), old, "one")["hardware"] == {}
...
def _strings():
    ...
    assert strings == english, "strings.json and translations/en.json have drifted apart"
    return strings
...
def test_every_setup_rule_has_a_message_in_both_flows():
    ...
    keys |= {f"plumbing_{verb}_{part}" for verb in ("needs", "unused") for part in ("pump", "mainline")}
    ...
    keys |= set(re.findall(r'(?:: |problem = )"([a-z_]+)"\}?', _flow_source())) & {
        "must_read_off", "setup_invalid_detail", "sizing_range", "calibration_no_entity"}
    keys |= {"catch_test_invalid", "catch_test_out_of_range"}
    assert {"must_read_off", "sizing_range", "calibration_no_entity"} <= keys  # the scan still works
    strings = _strings()
...
def test_every_wizard_field_has_a_label_a_tooltip_and_every_dropdown_its_options(flow_module):
    strings = _strings()
    fm = flow_module
    forms = {
        ...
        ("config", "substrate"): fm._substrate_schema({}, "L", "L/hr"),
    ...
    for (section, step), schema in forms.items():
        ...
        assert not fields - set(text["data"]), f"{section}.{step}: no label for {sorted(fields - set(text['data']))}"
        untold = fields - set(text["data_description"])
        assert not untold, f"{section}.{step}: no tooltip for {sorted(untold)}"
    for key, options in (("plumbing", api.PLUMBING_LAYOUTS), ("ec_unit", fm.EC_UNIT_CHOICES),
                         ("volume_unit", fm.VOLUME_UNITS), ("flow_unit", fm.FLOW_UNITS),
...
def test_tooltips_name_the_units_a_newcomer_has_to_choose_between():
    ...
    ec_help = steps["zones"]["data_description"]["ec_unit"] + steps["zones"]["data_description"]["zone_1_ec"]
