...
2. [Start the Crop Steering config flow](https://my.home-assistant.io/redirect/config_flow_start/?domain=crop_steering). Select manual setup for a new installation. The wizard takes about five short screens, every box has an explanation under it, and a problem is shown beside the box that caused it without losing what you typed. Existing environment-import installations remain supported. What it asks:
   - **Your system.** How many zones, and *how water reaches your plants*. If one switch is your whole watering system (typically a pump on a smart plug in a tent), choose **Zone switch only** and you will not be asked for a pump or main-line valve. This needs controller 0.15.0 or newer. It also asks whether you think in litres or US gallons, and L/hr or GPH, starting from your Home Assistant's own setting.
   - **Map your zones.** The switch, moisture probe(s) and EC probe(s) for each zone. Every mapped switch must be **OFF** at this moment; the wizard says so, and whether the switch is on or merely offline. Moisture must be a percentage. EC may be mS/cm, dS/m, µS/cm or CF and is converted for you; a probe that reports plain "ppm" or no unit needs the *EC unit* box at the top set (500 or 700 scale is printed on the meter).
   - **Plumbing and lights.** Only the shared switches your layout has, and your lights-on / lights-off hours on the 24-hour clock.
   - **What your plants are in.** Pick a block or pot from the list (or type a volume **per plant**), the rating of one dripper, and drippers per plant. If you have measured your drippers, tick the catch-test box and the next screen works out the real flow. Unsure of field capacity? Leave 70: after a few days each zone shows a *Suggested Field Capacity*.
   - **Optional extras.** Feed-water EC/pH probes (these switch on the source-water check), tank and room sensors. Press Submit to skip them all.
