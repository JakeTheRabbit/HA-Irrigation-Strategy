...
def test_min_daily_floor():
    ...
    assert (
        fire(S(phase="P2", vwc=50, ec=6, ec_smooth=6, daily_vol=12), p) is False
    )  # floor met -> no fire
    assert (
        fire(S(phase="P3", vwc=50, ec=6, ec_smooth=6, daily_vol=3, lights_on=False), p)
        is False
    )  # lights-off -> never
    assert (
        fire(S(phase="P2", vwc=69, ec=6, ec_smooth=6, daily_vol=3), p) is True
    )  # SENSOR-INDEPENDENT: fires even when "wet"
    assert (
        fire(S(phase="P2", vwc=95, ec=6, ec_smooth=6, daily_vol=3), p) is False
    )  # drown ceiling (>=90) holds -> never flood
    assert (
        fire(
            S(phase="P2", vwc=50, ec=6, ec_smooth=6, daily_vol=3, minutes_since_shot=2),
            p,
        )
        is False
    )  # spacing
    assert (
        fire(S(phase="P2", vwc=50, ec=6, ec_smooth=6, daily_vol=0), P()) is False
    )  # default min_daily=0 -> off
...
def test_anti_lockout_flush():
    _, _, f, _, r = decide(
        S(phase="P2", vwc=55, ec=10, feed_ec=3), P()
    )  # dilutive feed + room
    assert f is True and "FLUSH" in r
    _, _, f2, _, r2 = decide(
        S(phase="P2", vwc=55, ec=10, feed_ec=11), P()
    )  # non-dilutive
...
def test_watchdog_and_cap():
    assert (
        fire(
            S(phase="P2", vwc=40, minutes_since_shot=300, ec=6, ec_smooth=6),
            P(watchdog_hours=3),
        )
        is True
    )
    assert (
        fire(
            S(phase="P2", vwc=40, daily_vol=400, ec=6, ec_smooth=6),
    ...
    assert (
        fire(
            S(phase="P2", vwc=55, ec=10, feed_ec=3, daily_vol=400),
            P(max_daily_volume=300),
        )
        is True
    )
...
def test_helpers():
    ...
    out = cross_zone_outliers(
        {1: S(daily_vol=50), 2: S(daily_vol=48), 3: S(daily_vol=8)}
    )
...
def test_p2_anti_short_cycle():
    p = P()
    HOT = dict(
        phase="P2", vwc=55, ec=7.5, ec_smooth=7.5, feed_ec=3, peak_vwc=60
    )  # EC over 1.2x target, feed dilutive
    assert (
        fire(S(**HOT, minutes_since_shot=30), p) is True
    )  # enough gap -> dilute fires
    _, _, f, _, r = decide(S(**HOT, minutes_since_shot=3), p)
    assert (
        f is False and "dilute" not in r.lower()
    )  # too soon -> no machine-gun (the live bug)
    ...
    assert (
        decide(S(phase="P2", vwc=55, ec=10, feed_ec=3, minutes_since_shot=30), p)[2]
        is True
    )  # gap ok -> flush
