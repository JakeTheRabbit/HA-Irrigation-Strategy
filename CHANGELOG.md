...
Pair with controller **0.15.0**. A single-switch room needs **both**: on an older controller it sets up cleanly and is then held as "no hardware mapped", exactly as before. Existing rooms are not changed by updating: nothing the operator set moves, no entity changes id, and the controller carries straight on without a disarm cycle.

**🌱 In plain English**

- **The setup wizard tells you what is wrong while you can still fix it.** It used to take every answer across three screens, check them all at the very end, and if anything failed it closed with one line of text and threw your answers away. One grower got all the way through only to be told a switch "must read OFF". Problems now appear next to the box that caused them, on the screen you are on, and everything you typed stays put. Fix it, press Next again, carry on.
- **It says why a switch is not "off".** "Must read OFF" also covered a smart plug that was simply offline, which sent people hunting for a pump that was not running. It now says whether the switch is ON, unavailable, or missing.
- **A tent with one switch works.** If a single switch (usually a pump on a smart plug) is your whole watering system, the wizard asks "how does water reach your plants?", you pick "zone switch only", and you are never asked for a pump or main-line valve you do not have. Before, setup accepted this and then the controller refused to ever water it.
- **Use the units you have.** EC probes that report µS/cm, dS/m or CF are recognised and converted for you. A handheld "ppm" reading needs one extra answer (500 or 700 scale) because the same water reads 1000 on one pen and 1400 on another. Pot sizes can be typed in litres or US gallons and drippers in L/hr or GPH; it starts from whatever your Home Assistant is already set to, and every number box now shows its unit.
- **Less to look up.** Pick your block or pot from a list instead of working out litres. Run a catch test (water into a jug for a minute) and the wizard works out your real dripper flow, which is often 10-30% off the packet. Not sure of field capacity? Leave it: after a few days each zone shows a *Suggested Field Capacity* read from where it actually tops out.
- **Every box has a plain explanation**, including what the 24-hour lights times mean, that moisture must be a percentage, and that substrate volume is per plant, not per zone.
- **A brand-new install now actually works.** On a blank Home Assistant the controller could not find the room the wizard had just created, so it never watered. It had gone unnoticed because installs that have been running for a while were unaffected.
- **It loads on older Home Assistant.** On anything before 2026.5 the integration failed to start after setup, despite listing 2024.3 as supported.
- **Your lights times are used.** The wizard asked for lights-on and lights-off and then always used 12 and 0.
- **"Edit parameters" does something.** Changing a value under Configure saved it and then silently put the old value back.
- **A mapping can be removed.** Under Configure you could swap a sensor or pump for another but never clear it.
- **Honest labels.** The waste-valve box promised the valve is "forced closed during a shot". The controller has never operated it. It and four other boxes that feed nothing at runtime are now marked as recorded or display-only, and moved to an optional last screen.

**🔧 Technical notes**

- *Inline validation.* `setup_api.SetupError(ValueError)` carries `key`, `path` and `placeholders`; `str(error)` is the sentence callers already receive, so the dashboard, the `setup_*` services and the MCP server are unaffected. The config flow runs `prepare_setup` + the OFF check on **every** step against the answers so far and re-shows the form with `errors` on the offending field. `safety_report()` returns `{entity_id, reason, detail}` (on / unavailable / unknown / missing); `safety_blockers()` is derived from it and keeps the original sentence for the ON case. `configuration_payload` and `_update` carry the new fields.
- *Wizard steps.* `manual_zones` (zones, plumbing, units) → `zones` → `hardware` (only the switches the layout has, plus lights) → `substrate` → optional `catch_test` → `extras`. `last_step` labels the buttons. Optional entity fields are prefilled with `suggested_value` instead of `default`, which is what made them impossible to clear.
- *Plumbing layouts.* New optional entry field and descriptor attribute `plumbing`: `valves_only | pump_valves | mainline_valves | pump_mainline_valves`, required (no default) in the wizard. `prepare_setup` checks a declared layout both ways. **"No pump" is never inferred from an empty mapping**: a room that never declared a layout keeps the three-stage rule, so a pump mapping cleared by accident still holds loudly instead of opening valves with no water behind them. Configure offers an undeclared room the layout its mapped switches already imply.
- *EC units.* New pure `units.py`. `CropSteeringSensor._average_sensor_values(..., ec=True)` converts each probe from its own `unit_of_measurement` (mS/cm, dS/m, mmho/cm, µS/cm with either micro code point or ASCII u, S/m, CF, bare "EC") before averaging, so mixed-unit zones fuse correctly and the engine only sees mS/cm. Optional entry field `ec_unit` (`auto` default) applies **only** to probes reporting no unit or a bare `ppm`; a declared unit always wins. `feed_ec_factor` is worked out at setup time, stored on the entry and published in the descriptor (not re-derived live: an offline probe drops its unit attribute, and a flapping factor would read as a changed setup). `UNITS["ec"]` is gone.
- *Helpers.* New pure `setup_helpers.py`: `SUBSTRATE_PRESETS`, `catch_test_lph()` (wizard and new Configure → *Calibrate dripper flow*, which writes number entities only: no revision bump, no disarm) and `suggested_field_capacity()`. New per-zone `sensor.crop_steering_<prefix>zone_N_suggested_field_capacity` reads the controller's `learned_peak`; its +2 / floor 40 rule is pinned by a test to `setpoint_supervisor.py` so a suggestion never disagrees with what Auto Setpoints would set. Read-only.
- *Fix, blank installs.* `sensor`, `number`, `select` and `button` entities now set an explicit `entity_id`, as `switch` has since 2.17.1. Home Assistant ignores `_attr_object_id`, so a new install registered `sensor.engine_config`, `number.substrate_volume`, `sensor.zone_1_vwc`; the controller discovers rooms via `sensor.crop_steering_*engine_config` and reads `number.crop_steering_*`. An entity already in the registry keeps its id (tested with an operator-renamed entity).
- *Fix, HA < 2026.5.* `setup_panel` called `frontend.async_panel_exists`, added in Home Assistant 2026.5.0, unconditionally: `AttributeError` in `async_setup_entry`. It now falls back to the frontend's panel table, which is what that helper does. `tests/test_setup_panel.py` had hidden this by assigning the function onto its own stub.
- *Fix.* `PARAM_TO_ENTITY_KEY` lacked `lights_on_hour` / `lights_off_hour`. A seed only applies to a newly created entity, so an existing install's schedule cannot move (tested: wizard said 10-22, operator set 8-20, 8-20 survives).
- *Fix.* `edit_parameters` wrote `parameters`, reloaded, and each `RestoreEntity` number restored its previous value. It now sets the live number entities first, and shows live values rather than the stale copy recorded at setup.
- *Fix.* Global `substrate_volume` minimum 1.0 → 0.1 and `drippers_per_plant` maximum 6 → 20, matching the per-zone entities and `SIZING`. Abort reasons `not_env_config` / `reload_failed` and the `edit_features` menu entry had no translation.
- *In-place upgrade.* Every new field is additive and written only once declared: re-saving an old room is byte-identical, and its descriptor publishes no new keys. An unconvertible EC reading on an old install (no unit, bare ppm) passes through unchanged rather than being reinterpreted.
- *Tests.* New `tests_ha/` tier runs the integration inside a real Home Assistant (`pytest-homeassistant-custom-component`): fresh install through the real flow manager; that install handed to the **real add-on controller**, which adopts it and fires a single-switch shot; in-place upgrades from seeded snapshots (`tests_ha/fixtures/`) of a 2.17 wizard install and an env-file era install; Configure. Seven of the bugs above were reproduced against untouched `main` before being fixed. `tests/test_setup_wizard_rules.py` checks every form field has a label and tooltip and every error / abort / menu key has a message. The add-on suites are now hermetic (`F2_STATE_PATH`): on a machine with a writable `/data` they wrote a real `/data/state.json` and leaked it between tests.
- Not changed: the React *Rooms & setup* panel does not yet offer `plumbing` or `ec_unit` (rooms it creates stay three-stage). VWC must still be reported in %.
